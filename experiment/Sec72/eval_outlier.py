...
def eval_infl(setup, output_path, target_epoch, start_epoch, end_epoch, seed=0, gpu=0):
    ...
    # infl
    infl_ae = - joblib.load('%s/outlier_ae.dat' % (output_path,))
    infl_iso = joblib.load('%s/outlier_iso.dat' % (output_path,))
    np.random.seed(seed)
    infls = {'baseline':[], 'ae':infl_ae, 'iso':infl_iso}
    ...
    for k in k_list:
        ...
        # save
        fn = '%s/eval_epoch_%02d_to_%02d_outlier.dat' % (output_path, start_epoch, end_epoch)
